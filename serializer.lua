...
Serializer = (function()
	...
	local function createStatusText()
		...
		local ok, statusText = pcall(Drawing.new, "Text")
		if not ok or not statusText then
			return nil
		end
		...
		local function updateStatus(text)
			...
			local bounds = statusText.TextBounds or Vector2.zero
			statusText.Position = Vector2.new(viewport.X / 2 - bounds.X / 2, 50)
		end
	...
	local function serializeBinary(root,filename,saveSettings)
		...
		if not saveSettings.Clipboard and not saveSettings.Callback then
			...
			if saveSettings.Clipboard then
				if env.setrbxclipboard then
					env.setrbxclipboard(totalData)
				end
...
<ProtectedString name="Source">]==]..gsub(message, xmlReplacePattern, xmlReplace)..[==[</ProtectedString>
	...
	Serializer.Init = function(oldInd)
		...
		-- Potassium already supplies getbspval, so no legacy
		-- gethiddenproperty/base64 fallback is required.
	end
...
return {
	Init = function(oldindex)
		...
		env.getpcd = getpcd
		env.encodeBase64 = crypt and crypt.base64encode
		env.lz4compress = crypt and crypt.lz4compress
		env.hashmd5 = crypt and function(s)
			return crypt.hash(s, "md5")
		end
